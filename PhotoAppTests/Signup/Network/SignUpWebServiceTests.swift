...
class SignUpWebServiceTests: XCTestCase {
    ...
    func testSignupWebservice_WhenReceivedDifferentJSONResponse_ErrorTookPlace() {
        ...
        MockURLProtocol.stubResponseData = jsonString.data(using: .utf8)
        let expectation = self.expectation(description: "Signup() method expectation for a response that contains a different JSO structure")
        // Act
        sut.signup(withForm: signFormRequestModel) { (signupResponseModel, error) in
            
            // Assert
            XCTAssertNil(signupResponseModel, "The response model for a request containgin unknownJSON response, should have been nil")
    ...
    func testSignupWebservice_WhenEmptyURLStringProvided_ReturnsError() {
        // Arrange
        let expectaion = self.expectation(description: "An empty request URL string expectation")
    ...
    func testSignupWebservice_WhenURLRequestFails_ReturnsErrorMessageDescription() {
        ...
        sut.signup(withForm: signFormRequestModel) { signupResponseModel, error in
            // Assert
            XCTAssertEqual(error, SignupError.failedRequest(description: errorDescription), "Signup method did not return an expected error for the failed request")
